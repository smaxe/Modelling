...
ui <- fluidPage(
  ...
  sliderInput(inputId = "conds",
              label = "Choose number of conditions",
...
server <- function(input, output) {
  the_fun <- function(conds,times){
    ...
    return(plot(final_time,final_states,type="s",
                xlab="Relative Times", ylab = "State Transitions",
                main = "Simple Birth&Death Markov-Chain. A birth happens, when next state > current state and vice versa."))
  }
